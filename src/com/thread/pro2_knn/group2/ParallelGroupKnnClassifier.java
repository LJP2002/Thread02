...
public class ParallelGroupKnnClassifier {
    ...
    public String classify(Sample example){
        CountDownLatch endController=new CountDownLatch(numThreads);//计数器12->0
        //endCotroller.countDown();//-1
       ...
       /* for (Thread t:list){
        ...
        try {
            endController.await();  //相当于上面  join，阻塞，当这个countdownlatch中的值为0，则解除阻塞。
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        //4.排序距离
